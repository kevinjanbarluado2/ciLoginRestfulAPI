    ...
    function pdf_read_xref(&$result, $offset) {
        ...
        if ($xrefPos === false) {
            ...
            if (is_array($xrefStreamObjDec) && isset($xrefStreamObjDec[0]) && $xrefStreamObjDec[0] == PDF_TYPE_OBJDEC) {
                ...
                //$this->error(sprintf('This document (%s) probably uses a compression technique which is not supported by the free parser shipped with FPDI. Please only use pdf file version 1.4 and generate again.', $filename));
                $this->error(sprintf('Error merging your uploaded pdf (%s) to Insurance Plan pdf. Please only upload pdf file with version 1.4 for now and generate again.', $filename));
            } else {            
